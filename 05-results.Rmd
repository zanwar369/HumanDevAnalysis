...
Initially we pulled only the variables that have the least missing data as per Chapter 4. I then ran ggpairs across these variables to determine if they could be narrowed in selection. Given that the variables are all relatively highly correlated, it seems Education Index provides the clearest comparison in analysis of correlations across human development topics.
...
Next, in determining how to handle the year variable, we examine the yearly breakdown after 2010 (as the Inequality dataset only spans 2010-2018). From here, each year appears highly correlated (.985 to 1), suggesting years do not need to be examined individually, and can be aggregated with a simple average.
...
* Another cluster (Cluster 2) shows low "Inequality in life expectancy", high "Homelessness from natural disaster", and low "Homicide rate".  Cluster 2 shows a visible positive correlation between "Inequality in life expectancy" and "Homelessness from natural disaster", as well as a visible negative correlation between "Homelessness from natural disaster" and "Homicide rate".  These countries additionally show a positive relationship between "Homicide rate" and "Prison population".
...
* As remarked in the preceding parallel coordinate plot, a cluster of countries (Cluster 4) visually shows a positive relationship between "Inequality in life expectancy" and "Homelessness from natural disaster".
